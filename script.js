...
const PREVIEW_CONFIG = {
  ...
  lightA: { ...LIGHT_PRESETS["Dark Glass"].lightA },
  lightB: { ...LIGHT_PRESETS["Dark Glass"].lightB },
};

const previewConfig = {
  ...PREVIEW_CONFIG,
  lightA: { ...PREVIEW_CONFIG.lightA },
...
function createNumberLabel(number, anchorObject) {
  ...
  element.className = "number-label";
  element.textContent = number;
  element.dataset.id = String(number);
  element.dataset.anchor = anchorObject.name;
  element.style.backgroundColor = getLabelColor(number);
...
function createLabelsFromAnchors(root) {
  clearNumberLabels();

  const anchors = [];
  root.traverse((object) => {
    const match = object.name.match(LABEL_ANCHOR_PATTERN);
    if (!match) return;
